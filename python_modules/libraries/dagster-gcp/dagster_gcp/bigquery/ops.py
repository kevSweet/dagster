...
def bq_op_for_queries(sql_queries):
    ...
    def _bq_fn(context):
        ...
        for sql_query in sql_queries:
            ...
            context.log.info(
                "executing query {} with config: {}".format(sql_query, cfg.to_api_repr() if cfg else "(no config provided)")
            )
            results.append(
...
def _execute_load_in_source(context, source, source_name):
    ...
    context.log.info(
        "executing BQ load with config: {} for source {}".format(cfg.to_api_repr() if cfg else "(no config provided)", source)
    )
