...
class EmrJobRunner:
    ...
    def add_tags(self, log, tags, cluster_id):
        ...
        log.info(
            "Added EMR tags to cluster {}: {}".format(cluster_id, ", ".join(f"{tag}={value}" for tag, value in tags_items))
        )
    ...
    def run_job_flow(self, log, cluster_config):
        ...
        log.debug(
            "Calling run_job_flow({})".format(", ".join(f"{k}={v!r}" for k, v in sorted(cluster_config.items())))
        )
    ...
    def add_job_flow_steps(self, log, cluster_id, step_defs):
        ...
        log.debug(
            "Calling add_job_flow_steps({})".format(",".join((f"{k}={v!r}") for k, v in steps_kwargs.items()))
        )
